...
class SiamFCNet(nn.Module):
    def __init__(self, training=True):
        ...
        self.feature_map = nn.Sequential( #一个有序的容器，神经网络模块将按照在传入构造器的顺序依次被添加到计算图中执行，同时以神经网络模块为元素的有序字典也可以作为传入参数。
            nn.Conv2d(3,96,11,2),
            nn.BatchNorm2d(96),#进行数据的归一化处理，使数据在进行Relu前不会因为数据过大而导致网络性能的不稳定
